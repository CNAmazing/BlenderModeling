...
def main():
    ...
    poly_dict=read_json(os.path.join(input_folder,'data.json'))
    for image_path,basename in zip(jps_paths, basenames):
        
        # 处理每个图片
        Metric3d_Processing(onnx_model="checkpoint\Metric3d_small.onnx",
        ...
        for i in range(len(facade_result['boxes'])):
            ...
            # if class_id==0:
            #     facade_Params['window'].append(box)
            # elif class_id==1:
            #     facade_Params['balcony'].append(box)
            # elif class_id==2:
            #     facade_Params['door'].append(box)
        for key, value in facade_Params.items():
            if not value:
                print(f"{key}的检测结果：{value}")
